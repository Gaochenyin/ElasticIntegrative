...
library(elasticHTE)
```
Here, we illustrate the use of this package by reproducing the simulation studies in Yang et. al. (2022), Section 4.

The following codes layout the simulation setups.
```{r echo=TRUE}
m <- 2000 # size for RWE
...
```
Now, we generate the necessary datasets and conduct the proposed method for elastic integrative analysis as our simulation study.
...
```
After completing the simulation, we reproduce the simulation results in Yang et. al., Table 1.
```{r include=FALSE}
library(dplyr)
library(kableExtra)
...
load('summary_results.RData') # load the summary result, the raw data is stored in the data-raw folder
